...
11. **Rebalance mechanics final design (Task 8 follow-up — binding).** (a) FAB MASSES COME FROM CARGO: the entire minable pool is ≈0.148, so real fabs weigh ≤~0.05 — every earlier probe with mass-5 fabs was unphysical; fab-vs-planet capture chaos evaporates at real masses. ASSIST_K is retuned for real fab masses (~3.0, TUNE). (b) The assist force is a PD CONTROLLER: proportional restoring toward rNom plus a damping term derived from finite-difference deviation rates snapshotted per tick outside the integrator callback (contract-compliant; no vel reads) — eliminates the arrest-vs-overshoot knife edge. (c) SHIP ASSIST: `sim.setShipAssist(bodyName|null)` applies the same PD force with strength SHIP_ASSIST while the ship is stationed (Task 9: active during Return Slag); slag mass restoration stays vis-viva accretion (optimal but weak per unit mass — the PD force does the orbital healing, the mass does the budget/coupling healing). (d) Rescue and healing proofs must assert robustness across a small parameter GRID, not single points. (e) Task 10: SPHERE_MASS_REQUIRED must be set vs the 0.148 pool (~0.08-0.12), and fab placement near planets is safe at real masses. (Partly superseded by amendment 12: safety holds for saturn/jupiter-scale primaries, NOT mars.)
15. **Rendering lifecycle & dead-body meshes (Task 12 review — binding for Tasks 13/16).** (a) `Orrery` needs a `dispose()` (like Sky/Renderer) before Task 16's autoscale/restart can recreate it without leaking per-body geometries/materials/textures. (b) DEAD-BODY GHOST MESHES: Sim keeps catastrophe/fabLost bodies in `sim.bodies` and keeps integrating them (private `dead` Set, unexposed), so the Orrery renders and animates a "dead" body (e.g. a lost fab, or a collided moon) forever. Fix in Task 16: expose deadness from Sim (add a public `isDead(name)` or a `Body.alive` flag — a SANCTIONED small core change) so the Orrery can hide/fade the mesh and Task 13's camera won't dive to a corpse. Until then, restart-on-loss (Task 16) sidesteps it by rebuilding the whole scene. (c) `?demo` URL param (dev aid: advances sim while tab hidden) ships in main.ts — harmless, gated by exact param match.
14. **ASSIST_K=9000 & inside-orbit placement (Task 10 tuning — binding for Tasks 11-14).** ASSIST_K was raised 1500→9000 so cargo-sized counterweights (~0.003, vs Task 8's 0.03-0.05 test fabs) can hold a mined body; this is SAFE because the true bound is ASSIST_MAX=0.15 (unchanged) — the 6× only lets lighter/farther fabs reach the same cap. Verified: the real winning run stacks 6 counterweights near titan, all siblings green, no per-body cap needed. LATENT RISK for the UI tasks: the assist is only proven safe for counterweights placed radially OUTWARD of the body (as Director.suggestedBase/hasty do). If Task 11-14 ever exposes FREE player-chosen fab placement, INSIDE-orbit stacking at this gain can pull a body past nominal into runaway — gate free placement to outward-only, or add an inside-orbit guard, when that UI ships. Director.activeExtraction() is the public mining-mode signal for the HUD.
13. **Win-economy reality (Task 9 review — binding for Tasks 10/14).** The nominal minable m0 pool ≈ 0.15, but MARS (0.1045, ~73% of it) is a POISON TRAP: mining Mars in any mode fatally destabilizes its moon Phobos, which is unrescuable (Mars is a fab-exclusion zone, amendment 12a) → guaranteed catastrophe → loss. The honestly-accessible pool is Titan 0.0219 + Moon 0.0114 + Jupiter trio 0.0014 ≈ 0.035 usable after the 5% extraction floor. Consequences: (a) SPHERE_MASS_REQUIRED must be set vs the ~0.035 accessible pool (provisional 0.015; Task 10 measures achievable-under-best-play and sets final to ~55-65% of it, range [0.010,0.025]). (b) Mars is bodyRisk 'fragile' and selecting it emits a `riskWarning` DirectorEvent (Task 14 HUD surfaces it). (c) The extraction floor mines the PARTIAL dose to exactly EXTRACTION_FLOOR·m0 (no stranded mass; big-dt = small-dt). (d) V2 opportunity (out of scope now): a "consumed body" mechanic (mining a moon to its floor removes it from stability/catastrophe tracking) would unlock Mars by letting the player dismantle Phobos first — the thematically ideal fix, deferred to keep v1 shippable.
12. **Physics-core closure constraints (final Task 8 review — binding for Tasks 9-16).** (a) `addFab` returns Body | NULL — placements are REJECTED when fabMass/primary.mass > FAB_MASS_RATIO_MAX (~0.05; a 0.01 fab false-ambers mars, 0.05 ejects it — mars is effectively a fab-exclusion zone) or within FAB_MIN_SEPARATION (~8 u) of a live fab (closer pairs mutually capture, collide, and doom the rescue). Task 9's placement/suggestion logic must handle null and respect both. (b) A stationed counterweight is PERMANENT INFRASTRUCTURE: PD assist holds a rescued body at its envelope; removing assist lets the eccentric orbit re-express and the body can die — rescue ≠ re-circularization. Durable healing is ship-assist + slag in the sub-critical zone only. (c) Ship-only assist on a critical body = indefinite hold, not recovery ("hold while you build the fab"). (d) Per-body dose landscapes differ ~40×: phobos fatal at 0.25% m0 (vs titan 0.4%), flapping at 0.13%, and phobos is effectively UNRESCUABLE (mars fabs rejected/unstable) — Task 9 should warn on phobos mining; Task 10 must tune per-body, not per-titan. (e) tick() re-snapshots the PD pass every ≤2 tu internally and carries a fractional-DT accumulator — real-time frame loops are safe; headless bots may batch big ticks. (f) setShipAssist validates the body name.
